...
static Cvar::Cvar<int> g_bot_buildNumEggs("g_bot_buildNumEggs", "how many eggs bots should build", Cvar::NONE, 9);
static Cvar::Cvar<int> g_bot_buildNumTelenodes("g_bot_buildNumTelenodes", "how many telenodes bots should build", Cvar::NONE, 4);
...
static buildable_t chooseBuildableToBuild( gentity_t *self )
{
	...
	else if ( G_Team( self ) == TEAM_ALIENS )
	{
		...
		else
		{
			...
			if ( rand() < RAND_MAX * g_bot_buildProbEgg.Get() )
			{
				toBuild = BA_A_SPAWN;
			}
		}
...
AINodeStatus_t BotActionBuildInRadius( gentity_t *self, AIGenericNode_t *node )
{
	...
	{
		buildable_t toBuild = chooseBuildableToBuild( self );
